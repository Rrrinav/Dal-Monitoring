...
app.post("/upload", upload.fields([
  ...
  try {
    ...
    if (!uploadedFile) {
      return res.status(400).json({ error: "No file provided under the 'file' field." });
    }

    const metadataString = req.body.metadata;
    if (!metadataString) {
      return res.status(400).json({ error: "No metadata provided under the 'metadata' field." });
    }

    try {
      parsedMetadata = JSON.parse(metadataString);
      if (typeof parsedMetadata.lat !== 'number' || typeof parsedMetadata.lng !== 'number' || typeof parsedMetadata.accuracy !== 'number') {
        console.warn("Metadata structure mismatch. Received:", parsedMetadata);
      }
    } catch (parseError) {
      console.error("Failed to parse metadata JSON:", parseError);
      return res.status(400).json({ error: "Invalid JSON format for metadata field." });
    }
    ...
    if (err.code === 'NetworkingError') {
      ...
      errorMessage = 'AWS credentials are incorrect or S3/SQS policy access denied.';
    }
